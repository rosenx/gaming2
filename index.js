...
function cannonInterface() {
    ...
    if (contentEle.children.length === 0) {
        ...
        for (let towerObj of allTowerArr) {
            ...
            line.innerText = `Range;${towerObj.rangeR}px`;
            data.appendChild(line);
            line = document.createElement("p");
            line.innerText = `Bullet Speed:${towerObj.bullySpeed}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `Health:${towerObj.rangeR}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `Attack interval time:${towerObj.clock}`;
            data.appendChild(line);
            line = document.createElement("p");
            line.innerText = `price:${towerObj.price}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `More Information:${towerObj.comment}`;
            data.appendChild(line);
...
function monstersInterface() {
    ...
    if (contentEle.children.length === 0) {
        for (let monster of MONSTERS_FUNC_ARR_ALL) {
            ...
            let line = document.createElement("p");
            line.innerText = `Health:${monsterObj.maxHp}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `bonus:${monsterObj.addPrice}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `Collision damage:${monsterObj.colishDamage}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `speed:${monsterObj.speedNumb}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `Acceleration:${monsterObj.accelerationV}`;
            data.appendChild(line);

            line = document.createElement("p");
            line.innerText = `introduce:${monsterObj.comment}`;
            data.appendChild(line);
...
function endlessMode(mode, haveGroup = true) {
    ...
    function showInitPanel() {
        ...
        if (panelEle.innerHTML === "") {
            ...
            cancelBtn.innerText = "Cancel placement mode";
            ...
            addCommon.innerText = "If you are unable to place your turret and the canvas is flickering, please refresh your browser and try again.";
    ...
    function showSmallLevelUpPanel(thing, clickPos) {
        ...
        if (thing.levelUpArr.length === 0) {
            ...
            tips.innerText = 'This building is top-notch!';
        ...
        for (let levelUpItemFunc of thing.levelUpArr) {
            ...
            divLevelUpItemEle.addEventListener("click", () => {
                if (world.user.money >= divLevelUpItemEle.dataset.price) {
                    ...
                    let et = new EffectText("Not enough money!");
                    et.pos = clickPos;
                    world.addEffect(et);
                }
            });
        ...
        levelDownEle.addEventListener("click", () => {
            ...
            if (fatherFunc === null) {
                // 已经不能降级了
                let et = new EffectText("Can't downgrade!");
                et.pos = clickPos;
                world.addEffect(et);
            } else {
        ...
        textDiv.classList.add("inner-text");
        textDiv.innerHTML = `Sold<br>+${thing.price / 2}元`
    ...
    canvasEle.onclick = function (e) {
        ...
        if (addedThingFunc === null) {
            ...
            if (world.user.money < addedThing.price) {
                let et = new EffectText("No more money!");
            ...
            for (let item of world.getAllBuildingArr()) {
                if (addedThing.getBodyCircle().impact(item.getBodyCircle())) {
                    // 这里不可以放建筑
                    let et = new EffectText("You can't place a building here. Click on another location.");
                    et.pos = addedThing.pos.copy();
                    world.addEffect(et);
                    return;
                }
